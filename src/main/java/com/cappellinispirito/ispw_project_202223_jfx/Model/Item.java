...
public class Item {
    ...
    private int calculateScore(boolean isBeverage, float proteins, float fibers, float fruitPercentage, float calories, float sugars, float saturatedFats, float salt, List<String> additives, boolean isBiological) throws SQLException {
        ...
        if (nutriScore<-2) healthScore = 100;
            ...
            healthScore = (int) (0.00000281*Math.pow(nutriScore,7)-0.00017858*Math.pow(nutriScore,6) +0.00402463*Math.pow(nutriScore,5) -0.03479528*Math.pow(nutriScore,4) +0.02908120*Math.pow(nutriScore,3) +0.91405353*Math.pow(nutriScore,2) -7.98065470*nutriScore +81.31242712) * 3/5; //this is a plot function made by us which plots in 60ties.
            //logic for additives
